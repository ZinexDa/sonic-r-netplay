...
static void* run(void* args) {
    ...
    while(PROFILER_RUNNING){
        ...
        /* thd_sleep (not usleep) so the loop actually yields even when
         * recording is paused. Without yielding, the high-prio prof thread
         * starves main, so profiler_clean_up never gets to flip
         * PROFILER_RUNNING=false → thd_join hangs at exit. */
        thd_sleep(INTERVAL_IN_MS); // milliseconds
    }
...
void profiler_init(const char* output) {
	...
	main_attr.label = "DcProf";
    THREAD = thd_create_ex(/* 0 */&main_attr, run, NULL);
...
static bool write_samples(const char* path) {
    ...
    for(int i = 0; i < BUCKET_SIZE; ++i) {
        if(root->pc) {
            printf("Incrementing %u for %x. ", (unsigned int)((root->pc - lowest_address) / bin_size), (unsigned int) root->pc);
            bins[(root->pc - lowest_address) / bin_size]++;
            printf("Now: %d\n", (int) bins[(root->pc - lowest_address) / bin_size]);
